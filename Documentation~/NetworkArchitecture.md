...
**Device → Server**
- `register_device` — sent on every connection. Contains device ID, device model, app name, bundle ID, and the device secret. The server validates the secret and either registers or rejects the device.
...
**Server → Device**
- `registered` — device secret accepted, device is now in the lobby.
...
- Up to 6 simultaneous streams at 720p/2 Mbps each are supported
...
**VR device authentication — Device Secret**

VR devices have no user accounts and no login flow. Instead, every device sends a shared secret string (`DEVICE_SECRET`) in its `register_device` message. The server checks whether this matches the configured value. If it does not match, the connection is rejected immediately.

The device secret is a fleet-wide credential — every Quest headset in a deployment uses the same value. It is the VR Developer's responsibility to set the correct value in the Unity Inspector, and the Server Administrator's responsibility to keep it private.
...
Knowing the device secret is not sufficient on its own. Each new device that connects for the first time is placed in a `PendingApproval` state and rejected until the Server Administrator explicitly approves it in Settings → Devices. This prevents unknown hardware from connecting even if the device secret is somehow leaked. Approval is tied to the device's hardware ID (`SystemInfo.deviceUniqueIdentifier` on Android), which is stable across app reinstalls and only changes on factory reset.
